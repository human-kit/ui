...
**Actualización final (olas F4–F5, 2026-07-10): pendientes resueltos.** Validación: 1385 tests browser + 21 SSR en verde, svelte-check 0 errores (837 archivos).
- **Refactors estructurales completados:** máquina presence unificada (`primitives/presence.svelte.ts`, 5 copias eliminadas — se encontró una quinta en menu-overlay); motor de segmentos compartido (`datepicker/root/segment-engine.ts`, la comparación línea a línea mostró que las copias eran idénticas salvo la semántica de commit, preservada vía adapter); estado clock↔timepicker (`create-time-selection-state.svelte.ts`, −505 líneas, con paridad de `isInvalidDraft` y política de rango unificada); **los 6 contextos de stores-de-versión migrados a runas** (accordion, tabs, toggle-group, calendar, tree, table — patrón sistémico nº 10 cerrado; solo quedan contadores documentados para señales genuinamente event-like: medición DOM, focus-request one-shot, batching de estructura). dialog-trigger/popover-trigger: unificación evaluada y descartada con justificación.
- **Cola de bajos resuelta:** live region del button fuera del nombre accesible + `aria-busy` + `pressed` como override real; opt-out `region={false}` en accordion; resizers de table fuera del orden de Tab; tags del combobox visibles para AT (`aria-activedescendant`); scroll-lock `position:fixed` (iOS); hover-intent de 100ms en submenús + re-armado del safe-triangle; listbox `loop`/`typeahead`; checkbox `form`; dígitos full-width (IME); `Autocomplete.Empty` como option; `role="group"` en roots de pickers con label; segmentos con `aria-valuetext` "Vacío" localizado, `aria-valuemax` del día dinámico y re-entrada en overflow de dígito; clock `disabled` bloquea la rueda; portal reactivo `string | HTMLElement`; `cn(0)`; comparador de fechas con orden total; supresión de click residual de table operativa; listeners de documento solo con el overlay abierto.

**Cierre definitivo (ola final, 2026-07-11): cola residual completada.** Validación: **1479 tests browser + 21 SSR en verde, svelte-check 0 errores (848 archivos)**. Resueltos los ítems que restaban: `ComboBox.Status` (live region localizada) + triggers por teclado + diff real de selección; `minValue`/`maxValue` de primera clase en Calendar con triggers/teclado acotados; validación de días interiores en rangos tipeados + invalidación de caché de bounds; momentum táctil del clock + `"9:30"` normalizado; scrub-area saneado + redondeo simétrico + percent + `inputState` revalidado; keyboard-navigation (repeat sin trampa de Tab, typeahead APG cíclico, `activeElement` prioritario, `rovingTabindex.update` operativo); modalidad `virtual` revivida; `aria-hide-outside` con MutationObserver y exención de live regions; scroll-lock hasta el fin del exit; menu (`submenu-back`, listeners con refcount, `closeReason` reseteado); helpers del snippet de Dialog tipados + niveles de stack re-derivados; tree (`aria-selected` multiple invertido, claves normalizadas, IDs únicos entre instancias, warning dev); overflow-row/dropzone/label/locale-provider (bajos completos + README de SSR); foco virtual de autocomplete limpiado al blur; ids estrictos en listbox.

**Único ítem abierto (decisión de diseño del owner, no un bug):** semántica opcional `radiogroup`/`radio` para toggle-group en modo `single` — hoy anuncia toggle buttons independientes (patrón Radix, válido); implementar la variante radiogroup cambia la API de AT y requiere criterio de producto.
