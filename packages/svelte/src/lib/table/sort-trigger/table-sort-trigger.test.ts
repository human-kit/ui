...
describe('Table.SortTrigger', () => {
  it('keeps the header cell as the grid tab stop and toggles sort explicitly', async () => {
    ...
    await expect.poll(() => groupSortTrigger.getAttribute('data-sort-direction-state')).toBe('ascending');
  });
  ...
  it('keeps the snippet sortDirection in sync for controlled initial, toggle, and clear states', async () => {
    ...
    await expect.poll(() => groupSortTrigger.getAttribute('data-sort-direction-state')).toBe('descending');
    ...
    await expect.poll(() => groupSortTrigger.getAttribute('data-sort-direction-state')).toBe('none');
