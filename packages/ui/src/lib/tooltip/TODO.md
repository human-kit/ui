...
- [x] [S][P2][Area: Interaction][Owner: Unassigned][Target: Done] Keep the tooltip open while the pointer crosses the gap in a straight line, without a close delay.
- [x] [S][P2][Area: Interaction][Owner: Unassigned][Target: Done] Open on a long press for a touch pointer, behind `openOnLongPress`.
- [x] [C][P3][Area: Position][Owner: Unassigned][Target: Done] Follow the pointer along one axis, for a tooltip on a wide element, with `followPointer`.
