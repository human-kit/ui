...
describe('Table column pinning', () => {
	it('sticks pinned header cells with offsets accumulated from preceding same-side pins', async () => {
		...
		await vi.waitFor(() => {
			expect(getByTestId('header-b').style.left).toBe('60px');
		});
		...
		expect(headerA.style.left).toBe('0px');
		...
		expect(headerB.style.left).toBe('60px');
		...
		expect(headerD.style.right).toBe('0px');
	...
	it('sticks pinned body cells with the same offsets', async () => {
		render(TablePinnedTest);
		await vi.waitFor(() => {
			expect(getByTestId('cell-b-r1').style.left).toBe('60px');
		});
		...
		expect(cellD.style.right).toBe('0px');
	});
});
