...
describe('ComboBox', () => {
  ...
  describe('Automatic Filtering', () => {
    it('filters items based on input value', async () => {
      ...
      const filteredItems = screen.getByRole('listbox').element().querySelectorAll('[role="option"]');
  ...
  describe('Selection Behavior', () => {
    ...
    it('click on item selects it', async () => {
      ...
      const listbox = screen.getByRole('listbox').element();
      const firstOption = listbox.querySelector('[role="option"]:not([data-empty-placeholder])') as HTMLElement;
