...
<div class="w-full max-w-sm">
	<ComboBox.Root selectionMode="multiple" bind:value bind:inputValue>
		<div
			class="flex h-8 items-center gap-0.5 rounded-md border border-neutral-300 bg-white pr-1 pl-2 focus-within:outline-2 focus-within:-outline-offset-1 focus-within:outline-neutral-900 dark:border-neutral-700 dark:bg-neutral-900 dark:focus-within:outline-white"
		>
			<ComboBox.Input
		...
		<ComboBox.Tags class="mt-2 flex flex-wrap gap-1 empty:hidden">
			{#snippet children({ item })}
				<ComboBox.Tag
					class="inline-flex items-center gap-1 rounded-md bg-neutral-100 py-0.5 pr-1 pl-2 text-xs text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200"
				>
					{item.label}
					<ComboBox.TagRemove class="rounded hover:bg-neutral-200 dark:hover:bg-neutral-700">
						<X class="size-3.5" />
					</ComboBox.TagRemove>
				</ComboBox.Tag>
			{/snippet}
			...
			<ComboBox.List emptyPlaceholder="No fruits found">
				{#each filtered as fruit (fruit.id)}
					<ComboBox.Item
						...
						class="flex cursor-default items-center justify-between gap-2 rounded-sm px-2 py-2 text-sm text-neutral-900 outline-none hover:bg-neutral-100 data-[focused=true]:bg-neutral-100 data-[selected=true]:font-medium dark:text-white dark:hover:bg-neutral-800 dark:data-[focused=true]:bg-neutral-800"
					>
						{fruit.name}
