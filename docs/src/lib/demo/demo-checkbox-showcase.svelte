...
<div class="space-y-8">
	...
	>
		<div class="flex w-full max-w-sm flex-col gap-4">
			<div class="rounded-2xl border border-dashed border-gray-300 bg-white/70 p-5 dark:border-gray-600 dark:bg-gray-900/60">
				...
				<div class="mt-4 grid gap-2 rounded-xl bg-gray-50 p-3 text-xs text-gray-600 dark:bg-gray-800/80 dark:text-gray-300">
					<div class="flex items-center justify-between gap-3">
						<span>effectiveState</span>
						<span class="rounded-full bg-gray-200 px-2 py-0.5 font-medium text-gray-900 dark:bg-gray-700 dark:text-white">
		...
		<div class="grid w-full gap-6 lg:grid-cols-[minmax(0,18rem)_1fr]">
			<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
				...
				<div class="mt-4 grid grid-cols-3 gap-2">
					<button
						onclick={() => setVisualState('unchecked', (value) => (controlledChecked = value), (value) => (controlledIndeterminate = value))}
					...
					<button
						onclick={() => setVisualState('checked', (value) => (controlledChecked = value), (value) => (controlledIndeterminate = value))}
						class="rounded-lg border border-blue-600 bg-blue-600 px-3 py-2 text-sm text-white transition-colors hover:bg-blue-700"
					>
						Checked
					</button>
					<button
						onclick={() => setVisualState('indeterminate', (value) => (controlledChecked = value), (value) => (controlledIndeterminate = value))}
				...
				<div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
					...
					<div class="mt-3 grid gap-3 text-xs md:grid-cols-2">
						<div>
							...
							<pre class="rounded-xl bg-gray-50 p-3 text-gray-700 dark:bg-gray-800 dark:text-gray-200">{JSON.stringify(checkedEvents, null, 2)}</pre>
						</div>
						<div>
							<p class="mb-2 font-medium text-gray-700 dark:text-gray-300">onIndeterminateChange</p>
							<pre class="rounded-xl bg-gray-50 p-3 text-gray-700 dark:bg-gray-800 dark:text-gray-200">{JSON.stringify(indeterminateEvents, null, 2)}</pre>
						</div>
					</div>
				</div>
			</div>
	...
	>
		<div class="grid w-full gap-4 lg:grid-cols-2">
			<label class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-colors hover:border-blue-300 dark:border-gray-700 dark:bg-gray-900 dark:hover:border-blue-500">
				<div class="flex items-start gap-3">
					<Checkbox.Root
	...
	>
		<div class="grid w-full gap-6 lg:grid-cols-[minmax(0,22rem)_1fr]">
			<form onsubmit={handleSubmit} class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
				<div class="space-y-4">
					<label class="flex items-start gap-3">
						<Checkbox.Root
							bind:isChecked={playgroundChecked}
						...
						<span class="space-y-1">
							...
							<span class="block text-xs text-gray-500 dark:text-gray-400">
								Checked submits <code class="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-800">{formName}={formValue}</code>
							</span>
						</span>
					</label>
					...
					<div class="grid grid-cols-2 gap-2 text-sm">
						<button
							type="button"
							onclick={() => setVisualState('unchecked', (value) => (playgroundChecked = value), (value) => (playgroundIndeterminate = value))}
						...
						<button
							type="button"
							onclick={() => setVisualState('checked', (value) => (playgroundChecked = value), (value) => (playgroundIndeterminate = value))}
						...
						<button
							type="button"
							onclick={() => setVisualState('indeterminate', (value) => (playgroundChecked = value), (value) => (playgroundIndeterminate = value))}
			...
			<div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
				...
				<pre class="mt-3 overflow-auto rounded-xl bg-gray-50 p-4 text-xs text-gray-700 dark:bg-gray-800 dark:text-gray-200">{submittedEntries}</pre>
				<p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
					Unchecked and indeterminate both submit no entry because the hidden input only participates when checked.
				</p>
			</div>
		</div>
	</DemoSection>
		...
		<div class="grid w-full gap-6 lg:grid-cols-[minmax(0,16rem)_1fr]">
			<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
				<div class="flex items-center gap-3">
					...
					<div>
						<p class="text-sm font-medium text-gray-900 dark:text-white">Persisted indicator node</p>
						<p class="text-xs text-gray-500 dark:text-gray-400">Toggle to inspect stable DOM presence.</p>
					</div>
				</div>
			</div>
			...
			<div class="grid gap-4 md:grid-cols-2">
				...
				<div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
					<h3 class="text-sm font-semibold text-gray-900 dark:text-white">Production-style list row</h3>
					<div class="mt-3 space-y-3">
						{#each preferenceRows as row (row.id)}
							<label class="flex items-start gap-3 rounded-xl border border-gray-200 p-3 transition-colors hover:border-blue-300 dark:border-gray-700 dark:hover:border-blue-500">
								<Checkbox.Root
